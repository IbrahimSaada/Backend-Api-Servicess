...
namespace Backend_Api_services.Services
{
    public class NotificationService : INotificationService
    {
        ...
        public NotificationService(apiDbContext context, ILogger<QuestionController> logger)
        {
            _context = context;
        ...
        public async Task HandleAggregatedNotificationAsync(int recipientUserId, int senderUserId, string type, int? relatedEntityId, string action)
        {
            ...
            else
            {
                ...
                existingNotification.is_read = false;
                ...
                _context.Entry(existingNotification).Property(n => n.is_read).IsModified = true;
        ...
        public async Task HandleQuestionLikeNotificationAsync(int recipientUserId, int senderUserId, int questionId)
        {
            // Get recipient user and check notification settings
            var recipientUser = await _context.users.FindAsync(recipientUserId);
            ...
            // Check if there is an existing notification of type "QuestionLike" for this question
            var existingNotification = await _context.notification
                .FirstOrDefaultAsync(n => n.recipient_user_id == recipientUserId &&
                                          n.type == "QuestionLike" &&
                                          n.related_entity_id == questionId);
            ...
            if (existingNotification == null)
            {
                ...
                var notification = new Models.Entities.Notification
                {
                    ...
                    message = $"{senderFullName} liked your question.",
                    created_at = DateTime.UtcNow,
                    is_read = false,
                    last_push_sent_at = DateTime.UtcNow,
                    aggregated_user_ids = senderUserId.ToString()
            ...
            else
            {
                ...
                if (userCount <= namesToDisplay)
                {
                    ...
                    message = $"{string.Join(" and ", userNames)} liked your question.";
                }
                else
                {
                    ...
                    int othersCount = userCount - namesToDisplay;
                    var displayedNames = userNames.Take(namesToDisplay);
                    message = $"{string.Join(", ", displayedNames)}, and {othersCount} others liked your question.";
                }
        ...
        public async Task HandleAnswerNotificationAsync(int recipientUserId, int senderUserId, int questionId, int answerId)
        {
            ...
            // Check if there is an existing "Answer" notification for this question
            var existingNotification = await _context.notification
                .FirstOrDefaultAsync(n => n.recipient_user_id == recipientUserId &&
                                          n.type == "Answer" &&
                                          n.related_entity_id == questionId);
            ...
            if (existingNotification == null)
            {
                ...
                var notification = new Models.Entities.Notification
                {
                    ...
                    type = "Answer",
                    related_entity_id = questionId,
                    message = $"{senderFullName} answered your question.",
                    created_at = DateTime.UtcNow,
                    is_read = false,
                    last_push_sent_at = DateTime.UtcNow,
                    aggregated_user_ids = senderUserId.ToString(),
                    aggregated_answer_ids = answerId.ToString()
                };
            ...
            else
            {
                ...
                // Deserialize user and answer IDs
                var userIds = existingNotification.aggregated_user_ids?.Split(',').Select(int.Parse).ToList() ?? new List<int>();
                var answerIds = existingNotification.aggregated_answer_ids?.Split(',').Select(int.Parse).ToList() ?? new List<int>();
                ...
                else
                {
                    ...
                    // Add or move the senderUserId and answerId to the front
                    if (!userIds.Contains(senderUserId))
                    {
                        userIds.Insert(0, senderUserId);
                    }
                    ...
                    if (userCount <= namesToDisplay)
                    {
                        message = $"{string.Join(" and ", userNames)} answered your question.";
                    }
                    else
                    {
                        int othersCount = userCount - namesToDisplay;
                        var displayedNames = userNames.Take(namesToDisplay);
                        message = $"{string.Join(", ", displayedNames)}, and {othersCount} others answered your question.";
                    }

                    // Update notification
                    existingNotification.message = message;
                    existingNotification.aggregated_user_ids = string.Join(",", userIds);
                    existingNotification.aggregated_answer_ids = string.Join(",", answerIds);
                    existingNotification.created_at = DateTime.UtcNow;
                    ...
                    _context.Entry(existingNotification).Property(n => n.message).IsModified = true;
                    _context.Entry(existingNotification).Property(n => n.aggregated_user_ids).IsModified = true;
                    _context.Entry(existingNotification).Property(n => n.aggregated_answer_ids).IsModified = true;
                    _context.Entry(existingNotification).Property(n => n.created_at).IsModified = true;
                    _context.Entry(existingNotification).Property(n => n.is_read).IsModified = true;
                         ...
                         DateTime.UtcNow - existingNotification.last_push_sent_at >= pushCooldown))
                    {
                        if (!string.IsNullOrEmpty(recipientFcmToken))
                        {
                            try
                            {
                                ...
                                {
                                    Token = recipientFcmToken,
                                    Title = userCount > 1 ? "New Answers" : "New Answer",
                                    Body = message
                                });
